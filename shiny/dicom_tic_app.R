...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("load_btn", "Load DICOM (pydicom decompresses automatically)"),
      hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$capture_roi, {
    ...
    tic <- numeric(n)
    for (i in seq_len(n)) {
      # Extract frame i with correct dimensions
      if (length(d) == 3) {
        # grayscale: frames x h x w
        frame_2d <- frames[i, , ]
      } else if (length(d) == 4) {
        # color: frames x h x w x c
        frame_3d <- frames[i, , , ]
        ...
        showNotification(paste("Unexpected frame dimensions:", paste(dim(frame_2d), collapse=",")), type="error")
        return()
      }
  ...
  output$frame_plot <- renderPlot({
    ...
    # Extract frame with correct number of dimensions
    d <- dim(rv$frames)
    if (length(d) == 3) {
      # grayscale: frames x h x w
      frame <- rv$frames[idx, , ]
      ...
      plot.new(); text(0.5, 0.5, "Invalid frame dimensions")
      return()
    }
    
    # Handle both grayscale and color images
    if (length(dim(frame)) == 3 && dim(frame)[3] >= 3) {
      # RGB color image - convert to matrix for display
      img <- frame
      ...
      plot(0, type = "n", xlim = c(0, w), ylim = c(h, 0), xaxs = "i", yaxs = "i", xaxt = 'n', yaxt = 'n', ylab = '', xlab = '', asp = 1)
      
      # Create grayscale version for image display
      gray_img <- img[,,1]*0.299 + img[,,2]*0.587 + img[,,3]*0.114
      image(1:w, 1:h, t(gray_img), col = gray(0:255/255), add = TRUE, useRaster = TRUE)
    } else {
      # Grayscale image
      if (length(dim(frame)) == 3) {
        mat <- matrix(frame[,,1], nrow = dim(frame)[2], ncol = dim(frame)[3])
      } else {
        mat <- matrix(frame, nrow = dim(frame)[2], ncol = dim(frame)[3])
      }
      ...
      plot(0, type = "n", xlim = c(0, w), ylim = c(h, 0), xaxs = "i", yaxs = "i", xaxt = 'n', yaxt = 'n', ylab = '', xlab = '', asp = 1)
      image(1:w, 1:h, t(mat), col = gray(0:255/255), add = TRUE, useRaster = TRUE)
    }
