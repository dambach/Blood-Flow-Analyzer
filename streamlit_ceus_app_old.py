...
with st.sidebar:
    ...
    if st.button("Load DICOM", use_container_width=True, key="load_btn"):
        try:
            ...
            frames = ds.pixel_array.astype(float)
            ...
            # Normalize to 0-1
            frames = (frames - np.min(frames)) / (np.max(frames) - np.min(frames) + 1e-8)
            
            st.session_state.frames_raw = frames
            st.session_state.frames_cropped = frames.copy()
            st.session_state.crop_bounds = None
            st.session_state.flash_frame_idx = None
            st.session_state.roi_bounds = None
            st.session_state.tic_data = None
            ...
            st.success(f"✅ Loaded {frames.shape[0]} frames, shape: {frames.shape}")
        except Exception as e:
            st.error(f"Error: {e}")

if st.session_state.frames_raw is None:
    st.info("👈 Load a DICOM file to begin")
    st.stop()
...
with col1:
    ...
    # Choose colormap: Gray before crop, Hot (fire) after crop
    colormap = 'Hot' if frames is st.session_state.frames_cropped else 'Gray'
...
with col2:
    ...
    if st.button("✂️ Apply Crop", use_container_width=True, key="apply_crop_btn"):
        if st.session_state.pending_crop is not None:
            ...
            frames_orig = st.session_state.frames_raw
            
            # Crop all frames
            if frames_orig.ndim == 2:
                cropped = frames_orig[y0:y1, x0:x1]
                cropped = cropped[np.newaxis, :, :]
            elif frames_orig.ndim == 3:
                cropped = frames_orig[:, y0:y1, x0:x1]
            elif frames_orig.ndim == 4:
                cropped = frames_orig[:, y0:y1, x0:x1, :]
            ...
            st.session_state.generated_video_path = None
            st.session_state.generated_video_filename = None
            st.success(f"✅ Cropped to {cropped.shape}")
            st.rerun()
        else:
    ...
    if st.button("🔄 Reset Crop", use_container_width=True, key="reset_crop_btn"):
        st.session_state.frames_cropped = st.session_state.frames_raw.copy()
        st.session_state.crop_bounds = None
        st.session_state.roi_bounds = None
        st.session_state.tic_data = None
        st.session_state.crop_shape = None
